...
(async()=>{
  ...
  try{
    ...
    assert(await page.evaluate(async()=>(await import('./app.mjs')).game.discovered.has('nursery')));
    ...
    console.log(`PASS ${publicTest?'static /demo/ Pages path':'local site'} with default game_music.mp3: browser load, keyboard movement, pause/title, locked gates, secret room, teleport, map, focus loss and mobile layout`);
