...
let audioContext,muted=false,objectURL=null,hasMusic=false,lastMenu='',lastState='',musicPending=false,unlocked=false;
try{muted=localStorage.getItem('iron-signal-muted')==='true';}catch{}
function sound(name){if(muted||!audioContext)return;const values={shot:[160,.035],jump:[420,.08],hurt:[90,.12],pickup:[880,.1],warp:[640,.12],explode:[70,.16]};const [freq,duration]=name==='shot'?[game.weaponTier===3?900:game.weaponTier===2?600:420,.11]:values[name]||values.shot;
  const t=audioContext.currentTime,osc=audioContext.createOscillator(),gain=audioContext.createGain();osc.type='square';osc.frequency.setValueAtTime(freq,t);if(name==='shot')osc.frequency.exponentialRampToValueAtTime(65,t+duration);gain.gain.setValueAtTime(0,t);gain.gain.linearRampToValueAtTime(name==='shot'?.13:.045,t+.004);gain.gain.exponentialRampToValueAtTime(.0001,t+duration);osc.connect(gain);gain.connect(audioContext.destination);osc.start(t);osc.stop(t+duration+.01);osc.onended=()=>{osc.disconnect();gain.disconnect();};
}
function unlock(){unlocked=true;syncMusic();try{audioContext??=new (window.AudioContext||window.webkitAudioContext)();audioContext.resume().catch(()=>{});}catch{} }
function syncMusic(){audio.muted=muted;if(muted){audio.pause();return;}
  if(unlocked&&hasMusic&&audio.paused&&!musicPending){musicPending=true;audio.play().then(()=>{if(muted)audio.pause();}).catch(()=>{$('audio-status').textContent='Klicka i spelet för att starta musiken.';}).finally(()=>{musicPending=false;});}
}
function setMusic(url,name){audio.pause();audio.src=url;hasMusic=true;$('audio-status').textContent=`Musik: ${name} · spelas även i menyerna`;syncMusic();}
...
function syncUI(){
  ...
  if(menuKey!==lastMenu){
    ...
    if(game.state==='title'){
      ...
      actions=[button('Starta uppdraget · Enter',()=>game.enter(),true)];$('menu-help').textContent=game.levelIndex===5?'Eld är dödlig. THERMO BOOTS skyddar i 12 sekunder. Leta efter en lucka i elden och använd E.':game.levelIndex>=3?'Hitta färgade nyckelkort. E öppnar portar. Spruckna paneler döljer förråd.':'Samla 6 kapslar. Besegra kärnan. Nå EXTRACT. Välj bana med 1–6.';
...
window.addEventListener('keydown',e=>{
  ...
  const handled=controls[e.code]||['Escape','Enter','Tab','KeyP','KeyT','KeyR','KeyE','KeyM','KeyX','Digit1','Digit2','Digit3','Digit4','Digit5','Digit6'].includes(e.code);
