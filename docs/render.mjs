...
export class Renderer {
  ...
  sprite(name,x,y,flip=1,shirt=null){const c=this.c;c.save();c.translate(x,y);c.scale(flip,1);for(const q of this.data.sprites[name])draw(c,shirt&&q[5]==='rgba(232,78,82,1.000)'?[...q.slice(0,5),shirt,q[6]]:q);c.restore();}
  render(g){const c=this.c,l=g.level;if(this.index!==g.levelIndex){this.index=g.levelIndex;this.layers=[l.sky,l.backdrop,l.scenery].map(a=>new Layer(a));}
    ...
    for(const p of g.pickups)if((p.readyAt||0)<=g.time&&visible(p.x,p.y)&&(!p.secret||g.discovered.has(p.secret)))this.pickup(p,p.y+Math.sin(g.time*5+p.x)*3);
    ...
    c.globalAlpha=g.invincible>0&&Math.floor(g.invincible*14)%2===0?.35:1;this.sprite('hero',g.x,g.y,g.facing,g.armor>0?(g.armor>3?'#96d8ff':'#498bd9'):null);if(g.weaponTier>1)rect(c,g.x+g.facing*17,g.y+5,17,5,g.weapon.color);c.globalAlpha=1;
    if(g.fireproof>0){for(const dx of [-6,6])rect(c,g.x+dx,g.y-19,9,5,'#fff17b');}
  ...
  pickup(p,y){const c=this.c;if(p.kind<3){this.sprite(['cell','medkit','weapon'][p.kind],p.x,y);return;}
    ...
    else if(p.kind===7){for(const dx of [-8,8]){rect(c,p.x+dx,y,9,19,'#fff17b');rect(c,p.x+dx+2,y-8,13,6,'#ff974c');}text(c,'THERMO BOOTS',p.x,y+24,8,'#fff17b');}
    else{rect(c,p.x,y,21,25,'#62517b');rect(c,p.x,y,13,17,'#f9d375');rect(c,p.x,y,5,9,'#fff7c2');}
  }
  objects(g){const c=this.c;
    for(const [x,y,w,h] of g.level.fire||[]){rect(c,x+w/2,y+h/2,w,h,'#c13c27');
      for(let dx=0;dx<w;dx+=18){const flame=9+9*(1+Math.sin(g.time*8+dx*.17));rect(c,x+dx+8,y+h-4,18,9,'#ffab3e');rect(c,x+dx+8,y+h+flame/2,8,flame,'#ff783b');rect(c,x+dx+8,y+h+3,4,8,'#ffe486');}}
    for(const w of g.level.warps.filter(w=>w.requiresFire)){rect(c,w.a[0],w.a[1]-16,56,10,'#353e51');text(c,'[E] HEAT HATCH',w.a[0],w.a[1]+37,9,'#fff17b');}
  ...
  hud(g){const c=this.c;c.fillStyle='#101c2c';c.fillRect(0,0,W,48);c.fillRect(0,H-48,W,48);c.fillStyle=g.level.accent;c.fillRect(0,46,W,2);
    ...
    c.textAlign='right';c.fillStyle='#ffb34f';c.fillText(`${String(g.score).padStart(6,'0')} / ${g.levelIndex+1}-${g.levels.length} ${g.level.name}`,936,20);
    ...
    if(g.fireproof>0){c.fillStyle='#fff17b';c.fillText(`THERMO BOOTS ${Math.ceil(g.fireproof)}s`,W/2,H-35);}
  ...
  map(g){const c=this.c;c.fillStyle='rgba(4,9,19,.94)';c.fillRect(0,48,W,H-96);c.textAlign='center';c.font='bold 24px monospace';c.fillStyle=g.level.accent;c.fillText('MISSION MAP',W/2,94);
    ...
    for(const [x,y,w,h] of g.level.fire||[]){const [a,b]=p([x,y+h]);c.fillStyle='#f76b35';c.fillRect(a,b,w*scale,Math.max(3,h*scale));}
